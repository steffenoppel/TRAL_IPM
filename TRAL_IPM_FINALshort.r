...
# IPM runs from 2008-2021 # changed on 7 February - and again on 4 June 2021
...
#########################################################################
setwd("C:\\STEFFEN\\RSPB\\UKOT\\Gough\\ANALYSIS\\PopulationModel\\TRAL_IPM")
sink("TRAL_IPM_marray_age_recruit_immat_2008.jags")
cat("
...
#-------------------------------------------------
    ...
    ## ADJUSTED BASED ON PAST POPULATION SIZES WITH CHICK COUNTS SINCE 2003 (was 1999)
    ## MODIFIED TO START IN 2008

      IM[1,1,1] ~ dnorm(395,10)                                 ### number of 1-year old survivors is low because few chicks hatched in 2007
      IM[1,1,2] <- 0
      IM[1,1,3] <- IM[1,1,1] - IM[1,1,2]

      IM[1,2,1] ~ dnorm(491,10)                                 ### number of 2-year old survivors based on number of chicks hatched in 2006
      IM[1,2,2] <- IM[1,2,1]*p.juv.recruit.f[2]
      IM[1,2,3] <- IM[1,1,1] - IM[1,1,2]

      IM[1,3,1] ~ dnorm(388,10)                                 ### number of 3-year old survivors based on number of chicks hatched in 2005
      IM[1,3,2] <- IM[1,3,1]*p.juv.recruit.f[3]
      IM[1,3,3] <- IM[1,1,1] - IM[1,1,2]

      IM[1,4,1] ~ dnorm(239,10)                                 ### number of 4-year old survivors based on number of chicks hatched in 2004
      IM[1,4,2] <- IM[1,4,1]*p.juv.recruit.f[4]
      IM[1,4,3] <- IM[1,1,1] - IM[1,1,2]
    
      IM[1,5,1] ~ dnorm(203,5)                                 ### number of 5-year old survivors based on number of chicks hatched in 2003
      IM[1,5,2] <- IM[1,5,1]*p.juv.recruit.f[5]
      IM[1,5,3] <- IM[1,1,1] - IM[1,1,2]

      IM[1,6,1] ~ dunif(167,10)                                 ### very uncertain number of of 6-year old survivors based on number of chicks hatched in 2002
      ...
      Ntot.breed[1] ~ dnorm(1764,100)  ### sum of counts is 1764
      JUV[1] ~ dnorm(246,100)          ### sum of chicks is 246 in 2008
    ...
    for (tt in 2:T){
      ...
      for (age in 1:30) {
        logit(p.juv.recruit[age,tt])<-mu.p.juv[2] + eps.p[tt+28] + (agebeta * age)
      }
      ...
      IM[tt,1,1] ~ dbin(phi.juv[tt+28], max(1,round(JUV[tt-1])))                                  ### number of 1-year old survivors 
      IM[tt,1,2] <- 0
      IM[tt,1,3] <- IM[tt,1,1] - IM[tt,1,2]
    
      for(age in 2:30) {
        IM[tt,age,1] ~ dbin(phi.ad[tt+28], max(1,round(IM[tt-1,age-1,3])))
      ...
      N.ad.surv[tt] ~ dbin(phi.ad[tt+28], round(Ntot.breed[tt-1]+N.atsea[tt-1]))           ### previous year's adults that survive
      N.breed.ready[tt] ~ dbin(p.ad[tt+28], N.ad.surv[tt])                  ### number of available breeders is proportion of survivors that returns
...
TRALipm <- jags(jags.data, inits, parameters, "C:\\STEFFEN\\RSPB\\UKOT\\Gough\\ANALYSIS\\PopulationModel\\TRAL_IPM\\TRAL_IPM_marray_age_recruit_immat_2008.jags",
...
setwd("C:\\STEFFEN\\RSPB\\UKOT\\Gough\\ANALYSIS\\PopulationModel\\TRAL_IPM")
save.image("TRAL_IPM_output_v5_Ntot_agerecruit.RData")
