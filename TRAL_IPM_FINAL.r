...
# IPM runs from 2001-2021 # changed on 7 February
...
#########################################################################
setwd("C:\\STEFFEN\\RSPB\\UKOT\\Gough\\ANALYSIS\\PopulationModel\\TRAL_IPM")
sink("TRAL_IPM_marray_age_recruit_FINAL.jags")
cat("
    ...
    for (tt in 2:T){
      ...
      N3recruit[tt] ~ dbin(p.juv.recruit3[tt], round(N3[tt]))                                             ### number of 2-year old birds that return to Gough
      N4[tt] ~ dbin(phi.ad[tt+24], round(N3[tt-1]-N3recruit[tt-1]))                                                       ### number of 4-year old survivors
      N4recruit[tt] ~ dbin(p.juv.recruit4[tt], round(N4[tt]))                                             ### number of 2-year old birds that return to Gough
      N5[tt] ~ dbin(phi.ad[tt+24], round(N4[tt-1]-N4recruit[tt-1]))                                                       ### number of 4-year old survivors
      N5recruit[tt] ~ dbin(p.juv.recruit5[tt], round(N5[tt]))                                             ### number of 2-year old birds that return to Gough
      Noldrecruit[tt] ~ dbin(p.juv.recruit6[tt], round(N.notrecruited.surv[tt]))                                             ### number of 2-year old birds that return to Gough
...
for(scen in 1:n.scenarios){
    ...
    N.recruits.f[scen,tt] ~ dbin(mean.p.ad[2], round(N.notrecruited.surv.f[scen,tt]+N4.f[scen,tt]))                             ### number of this years recruiters
...
TRALipm <- jags(jags.data, inits, parameters, "C:\\STEFFEN\\RSPB\\UKOT\\Gough\\ANALYSIS\\PopulationModel\\TRAL_IPM\\TRAL_IPM_marray_age_recruit_FINAL.jags",
