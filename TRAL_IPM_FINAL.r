...
#########################################################################
setwd("C:\\STEFFEN\\RSPB\\UKOT\\Gough\\ANALYSIS\\PopulationModel\\TRAL_IPM")
sink("TRAL_IPM_FINAL_REV2022_minage.jags")
cat("
...
#-------------------------------------------------
    ...
    IM[1,2,2] <- 0 #IM[1,2,1]*p.juv.recruit.f[2]
    ...
    IM[1,3,2] <- 0 #IM[1,3,1]*p.juv.recruit.f[3]
  ...
  for(scen in 1:n.scenarios){
    ...
    for (tt in 2:FUT.YEAR){
      ...
      for(age in 2:3) {
        ...
        IM.f[scen,tt,age,2] <- 0 #min(round(IM.f[scen,tt-1,age-1,3]),IM.f[scen,tt,age,1])*p.juv.recruit.f[age]
...
jags.data <- list(marr.j = chick.marray,
                  ...
                  ### count data
                  n.sites=n.sites,
                  T = n.years,       ### -1 to exclude 2022 count
...
TRALipm <- run.jags(data=jags.data, inits=inits, parameters, 
                    model="C:\\STEFFEN\\RSPB\\UKOT\\Gough\\ANALYSIS\\PopulationModel\\TRAL_IPM\\TRAL_IPM_FINAL_REV2022_minage.jags",
