...
filepath <- "F:\\BethClark\\TRAL_IPM-main-2jul"
setwd(filepath)
...
sink("TRAL_IPM_marray_age_recruit_immat_v2_newpriors_17jul21.jags")
cat("
    ...
    agebeta ~ dnorm(0,1)    # Prior for shape of increase in juvenile recapture probability with age
...
#can't see all params in results
...
#TRALipm <- jags(jags.data, inits, parameters, "C:\\STEFFEN\\RSPB\\UKOT\\Gough\\ANALYSIS\\PopulationModel\\TRAL_IPM\\TRAL_IPM_marray_age_recruit_immat_v2.jags",
...
TRALipm <- run.jags(data=jags.data, inits=inits, parameters, 
                    model=paste0(filepath,
                                 "/TRAL_IPM_marray_age_recruit_immat_v2_newpriors_17jul21.jags"),
                    n.chains = nc, thin = nt, burnin = nb, adapt = nad,sample = ns, 
                    method = "rjparallel") 
#started 13.45pm 17/7/21 - computer blank
#started 10:30 18/7/21 - computer blank
#started 9:30 26/7/21
...
version <- paste0(Sys.Date(),"200000s") #nt <- 5;nb <- 1000; nad <- 500; nc <- 3; ns <- 2000 

save.image(paste0("TRAL_IPM_output_v5_Ntot_agerecruit_",version,".RData"))




load(("TRAL_IPM_output_v5_Ntot_agerecruit_2021-07-31200000s.RData"))
...
write.table(export,paste0(
  "TRAL_Gough_IPM_estimates_2021",version,".csv"),
  sep=",", row.names=F)
...
write.table(TABLE1,paste0(
  "TRAL_demographic_estimates_2021",version,".csv"),
  sep=",", row.names=F)
...
#todo
#change to colour-blind safe colourscheme
#rename legend labels

ggsave(paste0("TRAL_IPM_pop_trend_Gough_2004_2050_Ntot_beth_",
              version,".jpg"), width=14, height=8)

plot_seff <- ggplot(plot1_df) + 
  geom_line(aes(y=Median*2, x=Year, colour=factor(Scenario)), size=1) +   #
  geom_ribbon(aes(x=Year, ymin=lcl*2,ymax=ucl*2, fill=Scenario),alpha=0.3)+ #
  geom_point(data=TRAL.pop[TRAL.pop$tot>500 & TRAL.pop$tot<2395,],aes(y=tot*5, x=Year),col="firebrick", size=2.5)+
  geom_point(data=TRAL.pop[TRAL.pop$Year %in% c(2001,2011),],aes(y=tot*5, x=Year),col="salmon", size=2)+
  geom_line(aes(x=Year,y=SSeff, colour=factor(Scenario)),
...
ggsave(paste0("TRAL_IPM_pop_trend_Gough_2004_2050_Ntot_beth_",
              version,"_seff.jpg"), width=14, height=8)


save.image(paste0("TRAL_IPM_output_v5_Ntot_agerecruit_200000_plus_outputs.RData"))
