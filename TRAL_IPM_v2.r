...
J<- as.matrix(TRAL.chick[,c(2,4,9,10,11)])
#R[1,11]<- 350 ### fill in this so that there is no need to anchor the state-space model to an NA value
...
jags.data <- list(y = CH,
                  ...
                  ### count data
                  n.sites=n.sites,
                  prop=c(0.458,0.257,0.0261,0.0814,0.176),
...
parameters <- c("Ntot.breed","lambda","ann.fec","ann.surv","beta","mean.fec","mean.p")  
...
TRALipm <- jags(jags.data, inits, parameters, "C:\\STEFFEN\\RSPB\\UKOT\\Gough\\ANALYSIS\\PopulationModel\\TRAL_IPM\\TRAL_IPM_v2.jags", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb,parallel=T)
